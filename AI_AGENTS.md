# AI Agent Context — bppvcoach.com

## Summary

*one paragraph: what this site is, what it does*

The most common form of vertigo (BPPV) has a fix that takes under five minutes, but home attempts fail on one thing static videos can't help with: angle precision. bppvcoach.com is a free web tool that reads the phone's orientation sensors and shows live whether your head is hitting the Epley reference angles (45°/90°), validated to hold steady within a couple of degrees. It is deliberately positioned as an education and measurement aid, not a treatment device — keeping it clear of Software-as-a-Medical-Device classification and out of the brutal, authority-dominated "vertigo cure" search fight. It captures recurring sufferers in the calm period between episodes via a "save my profile" feature (their side, their PT-prescribed maneuver, their angle reference), owning the channel for when the next episode hits. Monetization (telehealth referral, ENT/PT white-label) is deferred until traffic and an email list exist.

## Audience

*one sentence: who this is for (broad demographic)*

Recurring BPPV sufferers, between episodes, who already self-administer the Epley maneuver from YouTube/Reddit/Facebook and are frustrated that home attempts miss.
...
Has had BPPV diagnosed or confirmed by a clinician at least once, so they already know the term and know their affected side from a prior PT or ENT visit. Episodes recur every few months. Comfortable enough with a phone to use a sensor tool, and already swapping maneuver videos in vertigo communities. Explicitly NOT the target: undiagnosed first-timers and people mid-attack — both are routed, in-product, to see a clinician (the tool never diagnoses which canal or which side, and never tells anyone to perform a maneuver).

## Goals

*1-2 sentences: primary business / product goal*

Rank for low-authority, between-episode long-tail queries within 90 days and become the bookmarked tool users return to when the next episode hits. Build an email list off the "save my profile" feature, with zero clinical interpretation anywhere in the product surface.
...
`proj=bppvcoach.com`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=bppvcoach.com` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=bppvcoach.com` | Start the dev server, skipping install. |
| `make test proj=bppvcoach.com` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=bppvcoach.com
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy bppvcoach.com"` runs `gh repo create` and
...
Tool-as-answer for the specific long-tail queries the tool literally resolves — "how do I know I'm at 45 degrees for Epley," "how to know if the Epley worked," "BPPV keeps coming back" — while honestly routing diagnostic queries ("which side is my BPPV") to a clinician (Dix-Hallpike) rather than answering them. Surrounding content is clinician-reviewed first-hand material structuring established protocol, never wholesale-generated medical claims, anchored by a vestibular PT (ideally VRC-credentialed) for E-E-A-T. Avoid the "vertigo cure" head term entirely — it's a Mayo/WebMD YMYL SERP and a trap. All content pages ship as static Astro (not CSR) so Googlebot can index them.
...
`portfolio project check bppvcoach.com` (run from `sites/portfolio/`).
